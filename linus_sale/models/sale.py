    ...
    def check_combine_constraint(self, combination_ids):
        if len(combination_ids) > 0:
            ...
            for combine_item in combine_item_list:
                ...
                if len(check_detail_by_item) == len(combine_number_list):
...
class SaleOrderLine(models.Model):
    ...
    def _onchange_discount(self):
        ...
        def combine_applied(lines, categories):
            if lines.product_id.categ_id == categories:
                return True
            return False

        def calculate_product_qty(lines, pricelist=False, combine_applied=False):
            qty = 0
            if pricelist.combination_ids and combine_applied:
                for line in lines:
                   if line.product_id.categ_id in pricelist.combination_ids.categ_id:
                        qty += line.product_uom_qty 
            else:
                for line in lines:
                    if line.product_id.categ_id == self.product_id.categ_id:
                        qty += line.product_uom_qty
            return qty
        ...
        if self.order_id.pricelist_id.apply_over:
            qty = calculate_product_qty(self.order_id._get_update_prices_lines(), 
                                        ...
                                        combine_applied(self.order_id._get_update_prices_lines(), self.order_id.pricelist_id.combination_ids.categ_id))
