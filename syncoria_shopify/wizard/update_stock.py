...
class ProductsFetchWizard(models.Model):
    ...
    fetch_type = fields.Selection([
        ...
        ('update_price', 'Update Price from Odoo To Shopify')
    ...
    def shopify_update_price_item(self):
        ...
        for product in products:
            time.sleep(1)
            for shopify_instance in self.shopify_instance_ids:
                prod_mapping = self.env['shopify.multi.store'].search(
                    [('product_id', '=', product.id),
                     ('shopify_instance_id', '=', shopify_instance.id)])
                marketplace_instance_id = shopify_instance
                ...
                try:
                    if prod_mapping:
                        if marketplace_instance_id.set_price:
                            variant = {"id": prod_mapping.shopify_id}
                            type_req = 'PUT'
                            ...
                            product_url = host + "/admin/api/%s/variants/%s.json" % (
                                api_version, prod_mapping.shopify_id)
                            _logger.info("Store Instance: %s" % marketplace_instance_id.name)
                            stock_item, next_link = Connector.shopify_api_call(
                                headers=headers,
                                url=product_url,
                            ...
                            if 'call_button' in str(request.httprequest) and stock_item.get('errors'):
                                ...
                                _logger.warning(_("Request Error: %s" % (errors)))
                                product.message_post(
                                    body='Update price {} on store {}.\n'.format(
                                        errors,
                                        marketplace_instance_id.name))
                            else:
                                product.message_post(
                                    body='Product {} ({}) updated price {} on store {}.\n'.format(
                                        product.name, product.default_code, product.lst_price, marketplace_instance_id.name))
                    else:
                        if marketplace_instance_id == product.shopify_instance_id:
                            if marketplace_instance_id.set_price:
                                variant = {"id": product.shopify_id}
                                ...
                                if 'call_button' in str(request.httprequest) and stock_item.get('errors'):
                                    ...
                                    product.message_post(
                                        body='Product {} ({}) updated price {} on store {}.\n'.format(
                                            product.name, product.default_code, product.lst_price,
                                            marketplace_instance_id.name))
                except Exception as e:
                    _logger.warning("Error in Request: %s" % e.args)
                    continue
