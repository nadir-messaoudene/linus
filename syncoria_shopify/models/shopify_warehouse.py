    ...
    def create_update_warehouse_to_odoo(self):
        ...
        try:
            for record in self:
                ...
                # warehouse_code = record.shopify_loc_name[0:2].upper()+"/"+record.shopify_invent_id[-3:]
                # exists_warehouse = warehouse_ids.search([("code","=",warehouse_code)])
                ...
                #         "shopify_warehouse_id": record.shopify_invent_id
                #     }
                #     warehouse_ids.create(warehouse)
                # else:
                #     exists_warehouse.update({"partner_id":partner.id})
                #     return {
                #         'type': 'ir.actions.client',
                #         'tag': 'display_notification',
                #         'params': {
                #             'title': "Warehouse Creation",
                #             'message': "%s warehouse already exists"%exists_warehouse.name,
                #             'type': 'warning',
                #         }
                #     }
                    # raise UserError("%s warehouse already exists"%exists_warehouse.name)
